...
export default function TeacherStudentGroupsPage() {
  ...
  return (
    <div>
      <PageHeader title="Nhóm học viên của tôi" description="Danh sách các lớp và nhóm luyện thi chứng chỉ bạn phụ trách" />
      ...
      <div className="mt-6 flex flex-col sm:flex-row gap-3">
        <SearchInput
          ...
          placeholder="Tìm kiếm nhóm theo tên, mã lớp..."
          maxLength={100}
        />
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-5">
        {loading ? (
          ...
          <div className="col-span-full py-16 text-center bg-surface-container-low rounded-2xl border border-outline-variant/30">
            ...
            <p className="text-sm text-on-surface-variant">Chưa có nhóm học viên nào</p>
          </div>
        ) : (
          groups.map((grp) => (
                ...
                {grp.members && grp.members.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-on-surface-variant mb-1.5">
                      Danh sách học viên ({grp.members.length}):
                    </p>
