    ...
    <tr>
      {[1, 2, 3, 4, 5].map((i) => (
        <td key={i} className="px-4 py-3">
          <div className="h-4 rounded bg-outline-variant/20 animate-pulse" style={{ width: `${50 + i * 10}%` }} />
        </td>
      ))}
    </tr>
  );
...
export default function TeacherLessonsPage() {
  ...
  return (
    <div>
      <PageHeader title="Bài học của tôi" description="Quản lý bài giảng bạn tạo" />
      ...
      <div className="mt-6 flex flex-col sm:flex-row gap-3">
        <SearchInput
          ...
          placeholder="Tìm kiếm bài học (tiêu đề, tóm tắt)..."
          maxLength={100}
        />
        <select
          value={status}
        ...
        {totalPages > 1 && (
          <div className="px-4 py-3 border-t border-outline-variant/30 flex items-center justify-between">
            <p className="text-xs text-on-surface-variant">
              {total} bài học · Trang {page}/{totalPages}
            </p>
