...
interface GoalOption {
  id: string;   // career goal code khớp với BE
  name: string;
  desc: string;
...
const goals: GoalOption[] = [
  { id: 'CLOUD_ARCHITECT', name: 'Thi đỗ chứng chỉ CNTT quốc tế (AWS, CompTIA, GCP)', desc: 'Luyện đề trắc nghiệm chuẩn format quốc tế và phản xạ từ vựng.', icon: 'military-tech' },
  { id: 'FULLSTACK_ENGINEER', name: 'Phỏng vấn xin việc & làm việc tại công ty Global', desc: 'Tự tin trình bày giải pháp kỹ thuật và trả lời câu hỏi Tech interview.', icon: 'work-outline' },
  { id: 'BACKEND_ENGINEER', name: 'Đọc hiểu tài liệu chính thống (Docs, RFCs, GitHub)', desc: 'Tăng tốc độ đọc hiểu các bản thiết kế kiến trúc và API references.', icon: 'auto-stories' },
  { id: 'SRE', name: 'Giao tiếp hàng ngày trong Scrum Team', desc: 'Viết PR review, ticket Jira, trao đổi trong Daily standup.', icon: 'forum' },
];

export default function OnboardingCareerGoalScreen() {
  const router = useRouter();
  const [selectedGoal, setSelectedGoal] = useState('CLOUD_ARCHITECT');

  const handleNext = async () => {
    await AsyncStorage.setItem('onboarding_career_goal', selectedGoal);
...
const styles = StyleSheet.create({
  ...
  goalName: { fontSize: 14, fontWeight: '700', color: colors.text, marginBottom: 2 },
  goalNameSelected: { color: colors.primary },
  goalDesc: { fontSize: 12, color: colors.mutedText, lineHeight: 16 },
